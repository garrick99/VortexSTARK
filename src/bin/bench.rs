...
fn main() {
    ...
    for log_n in [8u32, 12, 16, 20] {
        ...
        // warmup (2 runs)
        let _ = vortexstark::prover::prove(a, b, log_n);
        let _ = vortexstark::prover::prove(a, b, log_n);
        ...
        for _ in 0..iters {
            let t0 = Instant::now();
            let _proof = vortexstark::prover::prove(a, b, log_n);
            times.push(t0.elapsed().as_secs_f64() * 1000.0);
        }
        let stats = compute_stats(&mut times);
    ...
    for log_n in [8u32, 12, 16, 20] {
        ...
        let b = M31(1);
        let n_elem = 1u32 << log_n;
        let cache = vortexstark::prover::ProverCache::new(log_n);

        // warmup
        let _ = vortexstark::prover::prove_cached(a, b, &cache);
        let _ = vortexstark::prover::prove_cached(a, b, &cache);
        ...
        for _ in 0..iters {
            let t0 = Instant::now();
            let _proof = vortexstark::prover::prove_cached(a, b, &cache);
            times.push(t0.elapsed().as_secs_f64() * 1000.0);
        }
        let stats = compute_stats(&mut times);
        let proofs_per_sec = 1000.0 / stats.median;
    ...
    for log_n in [8u32, 12, 16, 20] {
        let n_elem = 1u32 << log_n;
        let pipeline = vortexstark::prover::ProverPipeline::new(log_n);
    ...
    for profile_log_n in [16u32, 20] {
        ...
        let _ = vortexstark::prover::prove_timed(M31(1), M31(1), profile_log_n);
        println!();
    }
    for profile_log_n in [16u32, 20] {
        println!("=== Stage Profile cached (log_n={profile_log_n}) ===");
        let cache = vortexstark::prover::ProverCache::new(profile_log_n);
        let _ = vortexstark::prover::prove_cached(M31(1), M31(1), &cache); // warmup
        let _ = vortexstark::prover::prove_cached_timed(M31(1), M31(1), &cache);
        println!();
    }
    ...
    for log_n in 8..=28u32 {
        ...
        let cache = match std::panic::catch_unwind(|| {
            vortexstark::prover::ProverCache::new(log_n)
        }) {
            Ok(c) => c,
            Err(_) => {
